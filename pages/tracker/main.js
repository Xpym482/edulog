...
$(document).ready(function(){
    ...
    if(Cookies.get('lesson_start')) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                response.forEach(function(log) {
                    createStopwatch(log['type'] + '_' + log['slug'], dateformatter(log['started_at']), false)
        ...
        lifeTime = setInterval(function() {
            ...
            $('#timer')[0].innerText = secondsToHMS(Math.floor((logTime.end - logTime.start) / 1000))
        }, 1000)
    }
    ...
    function createActivities() {
        ...
        Object.keys(structure[locale].groups).forEach(function (key) {
            ...
            $('#activities').append(`
                <div class="group" id="` + key + `">
                    <header class="group-header">
                        <h3>`+ structure[locale].groups[key].header +`</h3>
                    </header>
                    <div class="list"></div>
                </div>
            `)
            ...
            // create activities
            Object.keys(structure[locale].groups[key].activities).forEach(function (activity_key) {
                ...
                $('#'+key +' .list').append(
                    `
                    <a class="activity" id="`+ key +'_'+ activity_key +`">
                        <div class="bg" style="background-image: url('../../gradients/`+ backgrounds[key][activity_key]+`');">
                            <span class="timer">00:00:00</span>
                        </div>
                        <span>`+ structure[locale].groups[key]['activities'][activity_key] +`</span>
                    </a>
                    `
                )
            })
    ...
    function createStopwatch(key, started_at = new Date, request = false) {
        ...
        stopwatches[key] = {
            ...
            timer: setInterval(function() {
                ...
                $('#'+ key + ' .timer')[0].innerHTML = secondsToHMS(Math.floor((stopwatches[key].end - stopwatches[key].start) / 1000))
            }, 1000)
        }
    ...
    function reset() {
        ...
        $('#detailed_graph').children().remove()
