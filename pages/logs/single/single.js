$(document).ready(function() {
  ...
  function setupResultsView() {
    ...
    $(".activity").hover(
      function() {
        currentHoverBackground = $(".minibar")
          .eq($(this).index())
          .css("backgroundImage");
        $(".minibar")
          .eq($(this).index())
      ...
      function() {
        $(".minibar")
          .eq($(this).index())
          .css("background", currentHoverBackground);
  ...
  function makeDetailedGraph() {
    // total time
    var total =
      (dateformatter(lesson["ended_at"]) -
        dateformatter(lesson["started_at"])) /
      1000;
    console.log(total);
    $("#total_duration span").html(
      "Total duration: " + secondsToHMS(Math.floor(total))
    );
        ...
        `
          ...
          `">
                    <header class="group-header">
                        <h3>` +
          structure[locale].groups[group].header +
          `</h3>
                    </header>
                    <div class="list"></div>
                </div>
    ...
    Object.keys(grouped_by_slug).forEach(function(group) {
      Object.keys(grouped_by_slug[group]).forEach(function(slug) {
        ...
        grouped_by_slug[group][slug].forEach(function(log, index) {
          // calculate duration
          var part =
            (dateformatter(log["ended_at"]) -
              dateformatter(log["started_at"])) /
            1000;
          $(".bar#" + group + "_" + slug).append(
            jQuery("<div/>", {
              class: "minibar",
              css: {
                background:
                  "url('http://saargraafika.ee/edulog/gradients/" +
          ...
          $("#" + group + " .list").append(
            `
              ...
              `</span>
                            ...
                            <span>` +
              structure[locale].groups[group]["activities"][slug] +
              " - " +
